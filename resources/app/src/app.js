...
document.addEventListener("DOMContentLoaded", async () => {
  ...
  const tryAutoLoadRooms = async () => {
    ...
    if (!currentUser) {
      if (autoLoadRoomsAttempts < MAX_AUTO_LOAD_ATTEMPTS) {
        console.log(`⏳ Ожидание авторизации пользователя... (попытка ${autoLoadRoomsAttempts}/${MAX_AUTO_LOAD_ATTEMPTS})`);
        return;
      } else {
        console.warn('⚠️ Пользователь не авторизован после всех попыток, останавливаем автоматическую загрузку');
        if (autoLoadRoomsInterval) {
          clearInterval(autoLoadRoomsInterval);
          autoLoadRoomsInterval = null;
        }
  ...
  setTimeout(() => {
    ...
    if (!roomsListInitialized) {
      console.warn('⚠️ Автоматическая загрузка комнат не завершена за 15 секунд. Используйте кнопку обновления для ручной загрузки.');
    }
